...
public class CmdLIST extends CmdAbstractListing implements Runnable {
    ...
    public void run() {
        ...
        mainblock: {
            ...
            while (param.startsWith("-")) {
                ...
                Timber.tag(TAG).d("LIST is skipping dashed arg " + param);
        ...
        if (errString != null) {
            ...
            Timber.tag(TAG).d("LIST failed with: " + errString);
        } else {
            Timber.tag(TAG).d("LIST completed OK");
        }
    ...
    protected String makeLsString(File file) {
        ...
        if (!file.exists()) {
            Timber.tag(TAG).d("makeLsString had nonexistent file");
            return null;
        }
        ...
        if (lastNamePart.contains("*") || lastNamePart.contains("/")) {
            Timber.tag(TAG).d("Filename omitted due to disallowed character");
            return null;
        } else {

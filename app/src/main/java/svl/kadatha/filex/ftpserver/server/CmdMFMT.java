...
public class CmdMFMT extends FtpCmd implements Runnable {
    ...
    @Override
    public void run() {
        Timber.tag(TAG).d("run: MFMT executing, input: " + mInput);
        ...
        if (splitPosition == -1) {
            ...
            Timber.tag(TAG).d("run: MFMT failed, wrong number of parameters");
            return;
        }
        ...
        try {
            ...
            Timber.tag(TAG).d("run: MFMT failed, unable to parse parameter time-val");
            return;
        }
        ...
        if (!file.exists()) {
            ...
            Timber.tag(TAG).d("run: MFMT failed, file does not exist");
            return;
        }
        ...
        if (!success) {
            ...
            Timber.tag(TAG).d("run: MFMT failed, unable to modify last modification time");
        ...
        Timber.tag(TAG).d("run: MFMT completed successful");
    }

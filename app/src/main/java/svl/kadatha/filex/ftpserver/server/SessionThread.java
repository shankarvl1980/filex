...
public class SessionThread extends Thread {
    ...
    public boolean sendViaDataSocket(String string) {
        try {
            ...
            Timber.tag(TAG).d("Using data connection encoding: " + encoding);
            return sendViaDataSocket(bytes, 0, bytes.length);
        } catch (UnsupportedEncodingException e) {
            Timber.tag(TAG).d("Unsupported encoding for data socket send");
            return false;
        }
    }
    ...
    public boolean sendViaDataSocket(byte[] bytes, int start, int len) {
        ...
        if (dataOutputStream == null) {
            Timber.tag(TAG).d("Can't send via null dataOutputStream");
            return false;
        }
        if (len == 0) {
        ...
        try {
            ...
            Timber.tag(TAG).d("Couldn't write output stream for data socket, error:" + e);
            return false;
        }
    ...
    public int receiveFromDataSocket(byte[] buf) {
        ...
        if (dataSocket == null) {
            Timber.tag(TAG).d("Can't receive from null dataSocket");
            return -2;
        }
        if (!dataSocket.isConnected()) {
            Timber.tag(TAG).d("Can't receive from unconnected socket");
            return -2;
        }
        ...
        try {
            ...
            Timber.tag(TAG).d("Error reading data socket");
            return 0;
        }
        return bytesRead;
    }
    ...
    public boolean openDataSocket() {
        try {
            ...
            if (dataSocket == null) {
                Timber.tag(TAG).d("dataSocketFactory.onTransfer() returned null");
                return false;
            }
            ...
            Timber.tag(TAG).d("IOException getting OutputStream for data socket");
            dataSocket = null;
            return false;
        }
    }
    ...
    public void closeDataSocket() {
        Timber.tag(TAG).d("Closing data socket");
    ...
    public void quit() {
        Timber.tag(TAG).d("SessionThread told to quit");
        closeSocket();
    }

    @Override
    public void run() {
        Timber.tag(TAG).d("SessionThread started");
        ...
        try {
            ...
            while (true) {
                ...
                if (line != null) {
                    Timber.tag(TAG).d("Received line from client: " + line);
                    FtpCmd.dispatchCommand(this, line);
                } else {
                    Timber.tag(TAG).d("readLine gave null, quitting");
                    break;
                }
            }
        } catch (IOException e) {
            Timber.tag(TAG).d("Connection was dropped");
        }
        closeSocket();
    }
    ...
    public void writeBytes(byte[] bytes) {
        try {
            ...
            Timber.tag(TAG).d("Exception writing socket");
            closeSocket();
        }
    }
    ...
    public void writeString(String str) {
        ...
        try {
            ...
            Timber.tag(TAG).d("Unsupported encoding: " + encoding);
    ...
    public void authAttempt(boolean authenticated) {
        if (authenticated) {
            Timber.tag(TAG).d("Authentication complete");
            userAuthenticated = true;
        } else {
            authFails++;
            Timber.tag(TAG).d("Auth failed: " + authFails + "/" + MAX_AUTH_FAILS);
            if (authFails > MAX_AUTH_FAILS) {
                Timber.tag(TAG).d("Too many auth fails, quitting session");
                quit();
            }
        }
    }
    ...
    public void setWorkingDir(File workingDir) {
        try {
            ...
            Timber.tag(TAG).d("SessionThread canonical error");
        }
    }

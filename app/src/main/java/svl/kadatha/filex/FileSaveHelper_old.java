...
public class FileSaveHelper_old {
    ...
    public static SaveResult saveFile(Context context, Bundle bundle) {
        ...
        if (isWritable) {
            if (eol == alteredEol) {
                result = saveFile(null, prevPageEndPoint, currentPageEndPoint, content.getBytes(), file, temporaryFileForSave,currentPage,pagePointerHashmap);
            } else {
                result = saveFileWithAlteredEol(null, prevPageEndPoint, currentPageEndPoint, content, eolString, file, temporaryFileForSave,currentPage,pagePointerHashmap);
            }
        } else {
            fileOutputStream = FileUtil.get_file_outputstream(context, file.getAbsolutePath(), treeUri, treeUriPath);
            if (fileOutputStream != null) {
                if (eol == alteredEol) {
                    result = saveFile(fileOutputStream, prevPageEndPoint, currentPageEndPoint, content.getBytes(), file, temporaryFileForSave,currentPage,pagePointerHashmap);
                } else {
                    result = saveFileWithAlteredEol(fileOutputStream, prevPageEndPoint, currentPageEndPoint, content, eolString, file, temporaryFileForSave,currentPage,pagePointerHashmap);
                }
            } else {
                result = false;
            }
    ...
    private static boolean saveFile(FileOutputStream fileOutputStream, long prevPageEndPoint, long currentPageEndPoint, byte[] content, File file, File temporaryFileForSave,int current_page,LinkedHashMap<Integer, Long> pagePointerHashmap) {
        ...
        try {
            ...
            while (totalWritten < content.length) {
                int remaining = content.length - totalWritten;
                int toWrite = Math.min(remaining, CHUNK_SIZE);

                buf.clear();
                buf.put(content, totalWritten, toWrite);
                buf.flip();

                int written = sourceFC.write(buf);
                if (written != toWrite) {
                    throw new IOException("Failed to write entire chunk");
                }
            ...
            pagePointerHashmap.put(current_page,currentPageEndPoint);
    ...
    private static boolean saveFileWithAlteredEol(FileOutputStream fileOutputStream, long prevPageEndPoint, long currentPageEndPoint, String content, String eolString, File file, File temporaryFileForSave,int current_page,LinkedHashMap<Integer, Long> pagePointerHashmap) {

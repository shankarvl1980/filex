...
public class TextEditorSettingsDialog extends DialogFragment {
    ...
    public static TextEditorSettingsDialog getInstance(int selected_eol) {
        TextEditorSettingsDialog textEditorSettingsDialog = new TextEditorSettingsDialog();
        Bundle bundle = new Bundle();
        bundle.putInt("selected_eol", selected_eol);
        textEditorSettingsDialog.setArguments(bundle);
        return textEditorSettingsDialog;
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setCancelable(false);
        not_wrap = TextEditorActivity.NOT_WRAP;
        selected_text_size = TextEditorActivity.FILE_EDITOR_TEXT_SIZE;
        ...
        viewModel = new ViewModelProvider(requireActivity()).get(TextEditorViewModel.class);
    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.fragment_text_editor_settings, container, false);
        ...
        eol_rg.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            public void onCheckedChanged(RadioGroup rg, int p1) {
                if (!textEditorActivity.viewModel.fromThirdPartyApp && !textEditorActivity.viewModel.isFileBig) {
                    if (unix_rb.isChecked()) {
                        selected_eol = TextEditorActivity.EOL_N;
                    } else if (mac_rb.isChecked()) {
                        selected_eol = TextEditorActivity.EOL_R;
                    } else if (wnd_rb.isChecked()) {
                        selected_eol = TextEditorActivity.EOL_RN;
                    }
                } else {
                    switch (textEditorActivity.viewModel.eol) {
        ...
        ok_button.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
                if (TextEditorActivity.NOT_WRAP != not_wrap) {
                ...
                if (TextEditorActivity.FILE_EDITOR_TEXT_SIZE != selected_text_size) {
                    textEditorActivity.file_text_container_edittext.setTextSize(selected_text_size);
                    TextEditorActivity.FILE_EDITOR_TEXT_SIZE = selected_text_size;
                    textEditorActivity.tinyDB.putFloat("file_editor_text_size", TextEditorActivity.FILE_EDITOR_TEXT_SIZE);
                }

                ((TextEditorActivity) context).viewModel.altered_eol = selected_eol;

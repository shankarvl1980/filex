...
public class TextSaveHelper {
    ...
    private static final String TAG = "TextSaveHelper";
    ...
    public static SaveResult saveFile(Context context, Bundle bundle) {
        ...
        LinkedHashMap<Integer, TextEditorViewModel.PagePointer> pagePointerHashmap = getPagePointerHashmap(bundle);

        TextEditorViewModel.PagePointer currentPagePointer = pagePointerHashmap.get(currentPage);
        TextEditorViewModel.PagePointer prevPagePointer = pagePointerHashmap.get(currentPage - 1);
    ...
    private static LinkedHashMap<Integer, TextEditorViewModel.PagePointer> getPagePointerHashmap(Bundle bundle) {
        LinkedHashMap<Integer, TextEditorViewModel.PagePointer> pagePointerHashmap = new LinkedHashMap<>();
        try {
            Object obj = bundle.getSerializable("page_pointer_hashmap");
            if (obj instanceof HashMap) {
                @SuppressWarnings("unchecked")
                HashMap<Integer, TextEditorViewModel.PagePointer> hashMap = (HashMap<Integer, TextEditorViewModel.PagePointer>) obj;
    ...
    private static void updatePagePointers(LinkedHashMap<Integer, TextEditorViewModel.PagePointer> pagePointerHashmap,
                                           int currentPage) {
        Iterator<Map.Entry<Integer, TextEditorViewModel.PagePointer>> iterator = pagePointerHashmap.entrySet().iterator();
        while (iterator.hasNext()) {
            Map.Entry<Integer, TextEditorViewModel.PagePointer> entry = iterator.next();
    ...
    public static class SaveResult {
        ...
        public final LinkedHashMap<Integer, TextEditorViewModel.PagePointer> pagePointerHashmap;
        public final String errorMessage;

        public SaveResult(boolean success, LinkedHashMap<Integer, TextEditorViewModel.PagePointer> pagePointerHashmap, String errorMessage) {
